...
open class LabelLayout<Label: UILabel>: BaseLayout<Label>, ConfigurableLayout {
    ...
    private func textSize(within maxSize: CGSize) -> CGSize {
        ...
        size.height = RoundUtils.roundUpToFractionalPoint(size.height)
        size.width = RoundUtils.roundUpToFractionalPoint(size.width)
        if numberOfLines > 0 {
            let maxHeight = RoundUtils.roundUpToFractionalPoint(CGFloat(numberOfLines) * font.lineHeight)
